  ...
  String keyword = request.getParameter("t1");
      	...
      	try 
	{
	   ...
	   count++;
           ...
           String query="select * from document where dauthor LIKE '%"+keyword+"%'"; 
...
out.println("------------------------------------------------------------------------------------------------------------");
	...
	double unmatched_data=100.0000-matched_data;
		...
		st5.executeUpdate("insert into ratio(search_on,keyword,total,available ,matched_data,unmatched_data) values('Author','"+keyword+"','"+count+"','"+count1+"','"+matched_data+"','"+unmatched_data+"')" );
           connection.close();
		  ...
		  %>
				...
				<p style="color:#99FF00; font-size:22px"><br />
			    Out Preserving Triple data :<%=unmatched_data %> %</p>
		    <%
		  }
